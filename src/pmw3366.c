...
#define DT_DRV_COMPAT pixart_pmw3366
...
#include "pmw3366.h"
#include "pmw3366srom.h"
#include <zephyr/logging/log.h>

LOG_MODULE_REGISTER(pmw3366, CONFIG_PMW3366_LOG_LEVEL);
...
#define PMW3366_BURST_SIZE			6

/* Position of X in motion burst data */
#define PMW3366_DX_POS				2
#define PMW3366_DY_POS				4

/* Rest_En position in Config2 register. */
#define PMW3366_REST_EN_POS			5

#define PMW3366_MAX_CPI				12000
#define PMW3366_MIN_CPI				100


#define SPI_WRITE_BIT				BIT(7)

/* Helper macros used to convert sensor values. */
#define PMW3366_SVALUE_TO_CPI(svalue) ((uint32_t)(svalue).val1)
#define PMW3366_SVALUE_TO_TIME(svalue) ((uint32_t)(svalue).val1)
#define PMW3366_SVALUE_TO_BOOL(svalue) ((svalue).val1 != 0)
...
static int pmw3366_async_init_power_up(const struct device *dev);
static int pmw3366_async_init_configure(const struct device *dev);
static int pmw3366_async_init_fw_load_verify(const struct device *dev);
static int pmw3366_async_init_fw_load_continue(const struct device *dev);
static int pmw3366_async_init_fw_load_start(const struct device *dev);

static int (* const async_init_fn[ASYNC_INIT_STEP_COUNT])(const struct device *dev) = {
	[ASYNC_INIT_STEP_POWER_UP] = pmw3366_async_init_power_up,
	[ASYNC_INIT_STEP_FW_LOAD_START] = pmw3366_async_init_fw_load_start,
	[ASYNC_INIT_STEP_FW_LOAD_CONTINUE] = pmw3366_async_init_fw_load_continue,
	[ASYNC_INIT_STEP_FW_LOAD_VERIFY] = pmw3366_async_init_fw_load_verify,
	[ASYNC_INIT_STEP_CONFIGURE] = pmw3366_async_init_configure,
};
			     ...
			     size_t burst_size)
{
	...
	__ASSERT_NO_MSG(burst_size <= PMW3366_MAX_BURST_SIZE);
...
static int update_cpi(const struct device *dev, uint32_t cpi)
{
	...
	if ((cpi > PMW3366_MAX_CPI) || (cpi < PMW3366_MIN_CPI)) {
	...
	int err = reg_write(dev, PMW3366_REG_CONFIG1, value);
	if (err) {
				 ...
				 uint32_t time)
{
	...
	switch (reg_addr) {
	case PMW3366_REG_RUN_DOWNSHIFT:
		/*
		 * Run downshift time = PMW3366_REG_RUN_DOWNSHIFT * 10 ms
		 */
		maxtime = 2550;
		mintime = 10;
		break;

	case PMW3366_REG_REST1_DOWNSHIFT:
		/*
		 * Rest1 downshift time = PMW3366_REG_RUN_DOWNSHIFT
	...
	case PMW3366_REG_REST2_DOWNSHIFT:
		/*
		 * Rest2 downshift time = PMW3366_REG_REST2_DOWNSHIFT
			     ...
			     bool enable)
{
	...
	WRITE_BIT(value, PMW3366_REST_EN_POS, enable);
...
static int pmw3366_async_init_fw_load_start(const struct device *dev)
{
	...
	err = reg_write(dev, PMW3366_REG_CONFIG2, 0x00);
	if (err) {
	...
	err = reg_write(dev, PMW3366_REG_SROM_ENABLE, 0x1D);
	if (err) {
...
static int pmw3366_async_init_fw_load_continue(const struct device *dev)
{
	...
	err = reg_write(dev, PMW3366_REG_SROM_ENABLE, 0x18);
	if (err) {
	 ...
	 */
	err = burst_write(dev, PMW3366_REG_SROM_LOAD_BURST,
			  pmw3366_firmware_data, pmw3366_firmware_length);
	if (err) {
...
static int pmw3366_async_init_fw_load_verify(const struct device *dev)
{
	 ...
	 */
	uint8_t fw_id;
	err = reg_read(dev, PMW3366_REG_SROM_ID, &fw_id);
	if (err) {
		LOG_ERR("Cannot obtain firmware id");
		return err;
	}
	LOG_DBG("Optical chip firmware ID: 0x%x", fw_id);
	if (fw_id != PMW3366_FIRMWARE_ID) {
		LOG_ERR("Chip is not running from SROM!, returned fw_id is 0x%x", fw_id);
		return -EIO;
	}

	uint8_t product_id;
	err = reg_read(dev, PMW3366_REG_PRODUCT_ID, &product_id);
	if (err) {
		LOG_ERR("Cannot obtain product id");
		return err;
	}

	if (product_id != PMW3366_PRODUCT_ID) {
...
{
	int err;
	struct pmw3366_data *data = CONTAINER_OF(cb, struct pmw3366_data,
						 irq_gpio_cb);
...
static void trigger_handler(struct k_work *work)
{
	...
	int err = 0;
	struct pmw3366_data *data = CONTAINER_OF(work, struct pmw3366_data,
...
void pmw3366_debug_print_motion(const struct device *dev)
{
...
{
	/* Reset sensor */
	int reset = reg_write(dev, PMW3366_REG_POWER_UP_RESET, 0x5A);
	return reset;
}
...
static int pmw3366_async_init_configure(const struct device *dev)
{
	...
	err = update_cpi(dev, CONFIG_PMW3366_CPI);

	if (!err) {
		err = update_downshift_time(dev,
					    PMW3366_REG_RUN_DOWNSHIFT,
					    CONFIG_PMW3366_RUN_DOWNSHIFT_TIME_MS);
	}

	if (!err) {
		err = update_downshift_time(dev,
					    PMW3366_REG_REST1_DOWNSHIFT,
					    CONFIG_PMW3366_REST1_DOWNSHIFT_TIME_MS);
	}

	if (!err) {
		err = update_downshift_time(dev,
					    PMW3366_REG_REST2_DOWNSHIFT,
					    CONFIG_PMW3366_REST2_DOWNSHIFT_TIME_MS);
	}
...
static int dummy_read(const struct device *dev)
{
	...
	err = reg_read(dev, PMW3366_REG_MOTION, &motion);
	if (err) {
...
static int pmw3366_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (!err) {
		int16_t x = sys_get_le16(&buf[PMW3366_DX_POS]);
		int16_t y = sys_get_le16(&buf[PMW3366_DY_POS]);

		if (IS_ENABLED(CONFIG_PMW3366_ORIENTATION_0)) {
			data->x = -x;
			data->y = y;
		} else if (IS_ENABLED(CONFIG_PMW3366_ORIENTATION_90)) {
			data->x = y;
			data->y = x;
		} else if (IS_ENABLED(CONFIG_PMW3366_ORIENTATION_180)) {
			data->x = x;
			data->y = -y;
		} else if (IS_ENABLED(CONFIG_PMW3366_ORIENTATION_270)) {
			data->x = -y;
			data->y = -x;
		}
	}
...
static void pmw3366_async_init(struct k_work *work)
{
	struct pmw3366_data *data = CONTAINER_OF(work, struct pmw3366_data,
						 init_work.work);
	const struct device *dev = data->dev;

	LOG_DBG("PMW3366 async init step %d", data->async_init_step);

	data->err = async_init_fn[data->async_init_step](dev);
	if (data->err) {
		LOG_ERR("PMW3366 initialization failed");
	} else {
...
static int pmw3366_init(const struct device *dev)
{
	struct pmw3366_data *data = dev->data;
	const struct pmw3366_config *config = dev->config;
	int err;
	...
	err = pmw3366_init_irq(dev);
	if (err) {
		return err;
	}

	k_work_init_delayable(&data->init_work, pmw3366_async_init);
...
static int pmw3366_channel_get(const struct device *dev, enum sensor_channel chan,
...
static int pmw3366_trigger_set(const struct device *dev,
...
static int pmw3366_attr_set(const struct device *dev, enum sensor_channel chan,
			    ...
			    const struct sensor_value *val)
{
	...
	case PMW3366_ATTR_CPI:
		err = update_cpi(dev, PMW3366_SVALUE_TO_CPI(*val));
		break;

	case PMW3366_ATTR_REST_ENABLE:
		err = toggle_rest_modes(dev,
					PMW3366_REG_CONFIG2,
					PMW3366_SVALUE_TO_BOOL(*val));
		break;

	case PMW3366_ATTR_RUN_DOWNSHIFT_TIME:
		err = update_downshift_time(dev,
					    PMW3366_REG_RUN_DOWNSHIFT,
					    PMW3366_SVALUE_TO_TIME(*val));
		break;

	case PMW3366_ATTR_REST1_DOWNSHIFT_TIME:
		err = update_downshift_time(dev,
					    PMW3366_REG_REST1_DOWNSHIFT,
					    PMW3366_SVALUE_TO_TIME(*val));
		break;

	case PMW3366_ATTR_REST2_DOWNSHIFT_TIME:
		err = update_downshift_time(dev,
					    PMW3366_REG_REST2_DOWNSHIFT,
					    PMW3366_SVALUE_TO_TIME(*val));
		break;

	case PMW3366_ATTR_REST1_SAMPLE_TIME:
		err = update_sample_time(dev,
					 PMW3366_REG_REST1_RATE_LOWER,
					 PMW3366_REG_REST1_RATE_UPPER,
					 PMW3366_SVALUE_TO_TIME(*val));
		break;

	case PMW3366_ATTR_REST2_SAMPLE_TIME:
		err = update_sample_time(dev,
					 PMW3366_REG_REST2_RATE_LOWER,
					 PMW3366_REG_REST2_RATE_UPPER,
					 PMW3366_SVALUE_TO_TIME(*val));
		break;

	case PMW3366_ATTR_REST3_SAMPLE_TIME:
		err = update_sample_time(dev,
					 PMW3366_REG_REST3_RATE_LOWER,
					 PMW3366_REG_REST3_RATE_UPPER,
					 PMW3366_SVALUE_TO_TIME(*val));
		break;
...
static const struct sensor_driver_api pmw3366_driver_api = {
	.sample_fetch = pmw3366_sample_fetch,
	.channel_get  = pmw3366_channel_get,
	.trigger_set  = pmw3366_trigger_set,
	.attr_set     = pmw3366_attr_set,
};

#define PMW3366_DEFINE(n)						       \
	static struct pmw3366_data data##n;				       \
									       \
	static const struct pmw3366_config config##n = {		       \
		...
		.cs_gpio = SPI_CS_GPIOS_DT_SPEC_GET(DT_DRV_INST(n)),	       \
									       ...
									       \
	DEVICE_DT_INST_DEFINE(n, pmw3366_init, NULL, &data##n, &config##n,     \
			      POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY,	       \
			      &pmw3366_driver_api);

DT_INST_FOREACH_STATUS_OKAY(PMW3366_DEFINE)